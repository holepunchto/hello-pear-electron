...
| `CERTIFICATE_P12`       | Secret | `build-release.yml` | Base64-encoded macOS signing certificate bundle passed to the macOS build job.                | `MIIK...base64-p12...`                                |
| `CERTIFICATE_PASSWORD`  | Secret | `build-release.yml` | Password for the macOS `.p12` signing certificate.                                            | `correct-horse-battery-staple`                        |
| `MAC_CODESIGN_IDENTITY` | Secret | `build-release.yml` | macOS code signing identity name used by codesign.                                            | `Developer ID Application: Example Corp (ABCDE12345)` |
| `APPLE_ID`              | Secret | `build-release.yml` | Apple account email used for notarization.                                                    | `build-bot@example.com`                               |
| `APPLE_PASSWORD`        | Secret | `build-release.yml` | Apple app-specific password for notarization.                                                 | `abcd-efgh-ijkl-mnop`                                 |
| `APPLE_TEAM_ID`         | Secret | `build-release.yml` | Apple developer team ID for notarization.                                                     | `ABCDE12345`                                          |
